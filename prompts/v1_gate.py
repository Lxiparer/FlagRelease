#!/usr/bin/env python3
"""
v1_gate.py — 分支 B V1 二选状态机强制闸门判定（编排层专用，sglang 分支）

设计动机（与 step7_gate.py 同源）：
  分支 B 的 V1 基线必须由 baseline_selector.py 二选状态机确定性产出
  （v1.1/none，sglang 分支无 v1.2/v1.3），但 run_pipeline.sh 全程没有 shell 强制调用点——
  完全靠 Claude "记得"按 CLAUDE.md 调用。Claude 可能自起服务测一下就断言
  "V1 可跑/不可跑"，把 context 的 baseline.v1_variant/v1_available 写成臆断值，
  导致下游（V2 分支选择、合成基线触发、精度基线回退 NV）全部建立在
  未经二选验证的判据上。
...
判定输出（stdout 单行，供 shell case 分派）：
  ok      —— 已有 baseline_selector 真实产物，V1 二选已确定性执行过，放行
...
_VALID_VARIANTS = {"v1.1", "none"}   # sglang 分支二选
...
def has_real_selection(data):
    """v1_baseline_selection.json 是否含 baseline_selector.py 的真实二选痕迹。
    ...
    # 二选状态机必然从 v1.1 开始尝试；缺 v1.1 说明不是真实二选产物
...
def main():
    ap = argparse.ArgumentParser(description="分支 B V1 二选强制闸门判定")
