#!/usr/bin/env python3
"""
v1_gate.py — 分支 B V1 三选状态机强制闸门判定（编排层专用，sglang 分支）

设计动机（与 step7_gate.py 同源）：
  分支 B 的 V1 基线必须由 baseline_selector.py 三选状态机确定性产出
  （v1.1/v1.3/none，sglang 分支无 vllm 的 v1.2），但 run_pipeline.sh 全程没有 shell
  强制调用点——完全靠 Claude "记得"按 CLAUDE.md 调用。Claude 可能自起服务测一下就
  断言 "V1 可跑/不可跑"，把 context 的 baseline.v1_variant/v1_available 写成臆断值，
...
_VALID_VARIANTS = {"v1.1", "v1.3", "none"}   # sglang 分支三选
