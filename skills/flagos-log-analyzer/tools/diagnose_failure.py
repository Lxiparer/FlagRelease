...
def check_processes() -> Dict[str, Any]:
    """检查关键进程是否在运行。"""
    result = {"sglang": False, "eval": False, "benchmark": False, "details": []}
    try:
        ps = subprocess.run(["ps", "-eo", "pid,comm,args"], capture_output=True, text=True, timeout=5)
        for line in ps.stdout.splitlines():
            lower = line.lower()
            if "sglang" in lower and "serve" in lower:
                result["sglang"] = True
...
def infer_root_cause(
    ...
    if checkpoint:
        ...
        if any(k in action for k in ["gpqa", "eval", "benchmark", "perf"]):
            service_expected = True
    if service_expected and not service["running"] and not processes["sglang"]:
...
def format_human(diag: dict) -> str:
    ...
    running = [k for k in ["sglang", "eval", "benchmark"] if procs.get(k)]
    if running:
