...
- **非 gpqa 数据集预算**：mmlu 默认 20/子集 = 1140 题（57 子集 × 20），math_500 默认 40/等级 = 200 题。task_runner `--timeout` 按「题数 × 单题耗时（实测 1.5B 模型约 0.2-3s/题）× 1.25 缓冲」估算，如 mmlu 1140 题建议 ≥3600s，math_500 200 题 ≥3600s（慢速芯片需翻倍；--limit 100/--limit 0 取更大样本时相应上调）
