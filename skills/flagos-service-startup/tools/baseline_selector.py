...
# 冒烟测例抽离到独立工具 smoke_test.py（day0 快速适配流程与本三选状态机共用），
# SMOKE_PROMPT/SMOKE_KEYWORDS/smoke_test/resolve_service_port/resolve_served_model_id
# 均从该模块导入，勿在此重复定义。部署时两文件同目录（scripts/ 平铺）可正常 import。
from smoke_test import (SMOKE_PROMPT, SMOKE_KEYWORDS, smoke_test,
                        resolve_service_port, resolve_served_model_id)
